...
Als u van deze mogelijkheid gebruik wilt maken, moet de Maven-build een ZIP-bestand maken dat ten minste twee mappen bevat: ***conf*** en ***conf.d***. Dit ZIP-bestand kan worden gemaakt met de gefabriceerde insteekmodule. De projecten die door de Manager van de Wolk gebruikend de ingebouwde [tovenaar](create-an-application-project.md) worden geproduceerd hebben de correcte Maven die projectstructuur als deel van de projectverwezenlijking wordt gecreeerd. Dit is de geadviseerde weg voor nieuwe Beheerde klanten van de Diensten.

Bij plaatsing aan een **Instantie** van de dispatcher, zal de inhoud van deze folders de inhoud van deze folders op de instantie van de Verzender beschrijven. Aangezien de de configuratiedossiers van de Webserver en van de Verzender vaak milieu-specifieke informatie vereisen, om dit vermogen correct te gebruiken, zult u eerst met uw Ingenieurs van het Succes van de Klant (CSE) moeten werken om deze omgevingsvariabelen in ***/etc/sysconfig/httpd*** te plaatsen.

### Stappen voor het Vormen van Dispatcher voor Bestaande Beheerde Klanten van de Diensten {#steps-for-configuring-dispatcher}
...
1. Tot slot in het pom.xml- dossier in de wortel van uw project, voeg een `<module>` element toe om de verzender module te omvatten.
